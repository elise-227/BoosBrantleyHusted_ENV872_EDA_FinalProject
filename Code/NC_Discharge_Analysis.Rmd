...
DailyDischarge.Plot <- ggplot(NewRiver.Daily.DischargeGage, aes(Date, MeanDailyDischarge_cfs)) +
                      geom_line() +
...
NonSeasonal.DailyDischarge.ts <- ts(NonSeasonalDischarge.Daily$Seasonality_Removed,
                     start = c(1990, 06, 01), frequency = 153)
NonSeasonal.Plot <- autoplot(NonSeasonal.DailyDischarge.ts) +
                      scale_y_log10() +
                      labs(x = "Date", y = "Non-Seasonal Daily Discharge (cfs)", 
                           title = "Non-Seasonal Daily Discharge for New River, 
                           NC Hurricane Season (1990-2021)")
NonSeasonal.Plot
