...
The exploratory analysis of this data involved creating a map of the locations of the research gages along the East Coast of the United States was created to orient readers to the locality of the research. Also included n the exploratory analysis is the initial visualizations of discharge data of each of the research locations during hurricane seasons from 1990-2021. After initial graphs showed poorly visualized data a log scale was applied to the y-axis in order to better visualize any trend that may be occurring. Comparing differences between the intensity and frequency, and changes in these variables over time, allowed for the formation of predictions by researchers on possible differences in each research location as well as any changes over time. However, to fully understand these trends and trend changes the analysis that follows was performed. 

```{r map of locations, fig.cap='Locations of Research Gage Sites'}
...
The analysis of these datasets centers on creating time series objects of the discharge data. These time series objects are then decomposed to analyze the trend, seasonality, and remainder for the data values. New dataframes were then created with these three aspects of the time series. Within these dataframes, the trend and remainder values for each data point were added together to create a "non-seasonal" discharge value in order to better understand the actual changes in hurricane intensity and frequency during the hurricane season. 

These non-seasonal values were then used to create another time series object and this was graphed to visualize any trends that may be occurring in the data as well as the "spread" of the discharge values, indicating differences in discharge as a proxy for hurricane intensity. To analyze trends on a statistical basis, rather than visual, a Mann Kendall test was performed. This test provided a tau value indicating whether the trend was positive or negative, which could then be compared across all three research locations, as well as the p-value for that tau value to indicate the statistical significance of the results. 
...
There appears to be a poleward trend in hurricanes based on the trend in discharge values during hurricane months across the east coast. Hurricane discharge trends are increasing in New York and North Carolina while the last 30 years yielded a slightly decreasing trend in Florida. This poleward trend, with increasing hurricane frequency and intensity in states farther north could lead to exacerbated natural disasters. Large urban centers such as New York City are not equipped to handle such rain events, especially with the amount of impervious surface cover, which could lead to more disastrous flooding events. Future research on this topic could include more sites along the East Coast to get a better idea of the trend that may be occurring in terms of where hurricanes are becoming more/less frequent and intense. 
